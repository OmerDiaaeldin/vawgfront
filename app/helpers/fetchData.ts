...
import { LocationData, ProcessedLocation } from "../types";
import Papa from "papaparse";
export async function fetchData(
  setLocations: Dispatch<SetStateAction<ProcessedLocation[]>>,
    ...
    Papa.parse<LocationData>(text, {
      ...
      complete: (results) => {
        const validLocations = results.data
          .filter(
            (item) => item.Latitude && item.Longitude && item.id !== undefined
          )
          .map((item) => ({
            id: item.id,
            position: [
